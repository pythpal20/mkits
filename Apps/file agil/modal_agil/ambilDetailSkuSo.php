...
<div style="width:100%;overflow-x:auto">
    <div class="form-group">
        ...
        <input type="date" class="form-control" name="duedate" id="duedate" style="width:40%;">
    </div>
    <table class="table ">
        ...
        function ambilKeep($noso, $sku){
            ...
            $sql="SELECT no_keepstock,qty_req,qty_keep,status_keep,waktu_req,user,user_selesai FROM keepstock WHERE noso ='$noso' AND model ='$sku'";
            ...
            if ($result->num_rows > 0) {
                ...
                while($row = $result->fetch_assoc()) {
                    ...
                    array_push($qty_req,"<label class='label label-$warna'>  K-".add_leading_zero($row['no_keepstock']). " : [ ".date('y-m-d',strtotime($row['waktu_req']))." ] [".$row['user']."] req => ".$row['qty_req']. " pcs , keep =>" .$row['qty_keep']." pcs , status => ".status($row['status_keep']). " [" .$row['user_selesai']."]  </label><br/>");
                    // penjumlahan qty req
            ...
            <tr>
                ...
                <td class="text-center" style="min-width:150px;">
                    <div class="input-group">
                        <input type="number" placeholder='qty' id='input_<?=$dtl['id'];?>' class="form-control">
                        <span class="input-group-append">
                            <button type="button" class=" btn btn-primary test" qty="<?=$dtl['qty'];?>"
                                id="<?=$dtl['id'];?>" customer="<?=$hdr['customer_nama'];?>"
                                total_qty_req="<?=ambilKeep($dtl['noso'],$dtl['model'])['total_qty_req'];?>"
                                sku="<?=$dtl['model'];?>" noso="<?=$dtl['noso'];?>">
                                Keep
                            </button>
                        </span>
                    </div>
