...
$table = <<<EOT
 (
    SELECT distinct no_keepstock, customer,jenis_so,duedate,priority, noso FROM keepstock ORDER BY no_keepstock DESC
 ) temp
EOT;
...
$columns = array(
    ...
    array( 'db' => 'priority', 'dt' => 4, "formatter" => function ($d, $row){
        if($d =="urgent"){
            return "<label class='label label-danger'>".$d."</label>";
        }else if($d =="scheduled"){
            return "<label class='label label-primary'>".$d."</label>";
        }else{
            return "<label class='label label-warning'>not important</label>";
        }
    } ),
    array( 'db' => 'noso', 'dt' => 5 ),
    array( 'db' => 'jenis_so', 'dt' => 6 ),
    array( 'db' => 'no_keepstock', 'dt' => 7, 'formatter' => function( $d, $row ) {
    ...
    array( 'db' => 'no_keepstock', 'dt' => 8, 'formatter' => function( $d, $row ) {
    ...
    array( 'db' => 'no_keepstock', 'dt' => 9, 'formatter' => function( $d, $row ) {
