...
$table = <<<EOT
 (
    ...
    WHERE NOT EXISTS (SELECT * FROM customerorder_hdr_delivery e WHERE a.No_Co = e.No_Co) AND  (d.jenis_transaksi = 'MARKETPLACE')
    GROUP BY b.No_Co
 ) temp
EOT;
...
$columns = array(
    ...
    array( 'db' => 'sales', 'dt' =>7),
    array( 'db' => 'status_tanggal', 'dt' =>8, "formatter" => function ($d, $row){
        $status = explode("|", $d)[0];
        $tanggal = explode("|", $d)[1];
        if ($status== '0') {
