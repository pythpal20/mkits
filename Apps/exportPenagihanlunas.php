...
WHERE d.id_perusahaan LIKE '$perusahaan'
AND
((select sum(g.nominal) from detail_penagihan g where a.No_co = g.noco)-(select sum(b.harga_total) from customerorder_dtl_delivery b where b.No_co = a.No_Co)) >= 0
AND
DATE_FORMAT(g.tgl_input, '%Y-%m-%d') >= '$start' 
AND
...
if ($_POST['perusahaan'] == '1') {
  $perusahaan = 'Multi Wahana Kencana';
} elseif ($_POST['perusahaan'] == '2') {
  $perusahaan = 'Multi Wahana Makmur';
} elseif ($_POST['perusahaan'] == '3') {
  $perusahaan = 'Batavia Adimarga Kencana';
} elseif ($_POST['perusahaan'] == '4') {
  $perusahaan = 'Dewata Titian Mas';
} elseif ($_POST['perusahaan'] == '5') {
  $perusahaan = 'Food Container Indonesia';
} elseif ($_POST['perusahaan'] == '') {
  $perusahaan = 'Semua Perusahaan';
};
header("Content-Type:   application/vnd.ms-excel; charset=utf-8");
header("Content-Disposition: attachment; filename=laporan_penagihan_selesai($start sampai $end) $perusahaan.xls");  //File name extension was wrong
header("Expires: 0");
