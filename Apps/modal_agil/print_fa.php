...
<body>
	...
	JOIN customerorder_hdr d ON d.No_Co = a.No_Co 
	JOIN list_perusahaan b ON d.id_perusahaan = b.id_perusahaan 
	WHERE a.No_Co = '$id'";
	...
	$qry = "SELECT SUM(amount) AS tAmt, SUM(ppn) AS tPjk, SUM(diskon) AS tDsc, SUM(harga_total) AS total FROM customerorder_dtl_delivery WHERE No_Co = '$id'";
	$mwk = $db1->prepare($qry);
	$mwk -> execute();
	$hsl = $mwk->get_result();
	$dtl = $hsl->fetch_assoc();
	...
	JOIN customerorder_hdr_delivery b ON a.noso = b.noso
	WHERE b.No_Co = '$id'";
	$mwk = $db1->prepare($qry);
	$mwk -> execute();
	$hsls = $mwk->get_result();
	...
	$sqlquery = "SELECT * FROM customerorder_dtl_delivery a JOIN master_produk b ON a.model = b.model WHERE a.No_Co ='$id'";
...
?>
    <table border="0" width="100%">
        <tr>
            ...
            <td width="50%" style="text-align: right;vertical-align:top; ">
                <p>
                    ...
                    Order Date / Date sending Order :
                    <?= date_format(date_create($row['tgl_order']), "d/m/Y") .'/'.date_format(date_create($row['tgl_order']), "d/m/Y") ; ?>
                </p>
            </td>
        </tr>
        <tr>
        ...
        <tr>
            ...
            <td width="50%"
                style="text-align:left; vertical-align:top; border:1px solid black; border-collapse: collapes; padding: 5px">
        ...
        <table width="100%">
            <thead>
                <tr>
                    ...
                    <th>U.P (net)</th>
                    <th> Qty </th>
                    <th align="center">Total (Net of Tax)</th>
                </tr>
            </thead>
            <tbody style="font-size;0.820em;">
                <?php $no =0; while ($itm = $sql_hasil->fetch_assoc()) { $no++;?>
                <tr style="border:no-border;max-height:18px;">
                    <td style="border-style : hidden!important;vertical-align : top;width:30px;  " align="center">
                        <?=  $no; ?></td>
                    <td style="border-style : hidden!important;vertical-align : top;  ">
                        <?= $itm['model'] . '/' . $itm['deskripsi']; ?></td>
                    <td style="border-style : hidden!important;vertical-align : top;  " align="center">
                        <?= $itm['ppn']; ?></td>
                    <td style="border-style : hidden!important;vertical-align : top;" align="center">
                        <?= $itm['price']; ?></td>
                    <td style="border-style : hidden!important;vertical-align : top; " align="center">
    ...
    <table width="100%">
        <tbody>
            <tr>
                <td align="left" rowspan="5" width="40%" colspan="2"><?= $row['atasnama'] ;?><br> No. Rek
                    <?= $row['nama_bank'] ; ?>: <?=  $row['rekening']; ?>
                </td>
            </tr>
            <tr>
