        ...
        function ambilKeep($noso, $sku){
            ...
            if ($result->num_rows > 0) {
                ...
                while($row = $result->fetch_assoc()) {
                    ...
                    array_push($qty_req,"<label class='label label-$warna'>  K-".add_leading_zero($row['no_keepstock']). " : [ ".date('y-m-d',strtotime($row['waktu_req']))." ] [".$row['user']."] req => ".$row['qty_req']. " pcs , keep =>" .$row['qty_keep']." pcs , status => ".status($row['status_keep'])."  </label><br/>");
                    // penjumlahan qty req
