<?php
    ...
    $sql_dtl = "SELECT a.model, b.deskripsi, a.harga_aktual, a.qty_aktual, a.amount_aktual, a.ket
    FROM pts_detail_aktual a JOIN master_produk b ON a.model = b.model
    WHERE a.idPts = '$id' ORDER BY nourut ASC";
    $mwk = $db1->prepare($sql_dtl);
    $mwk -> execute();
    $hsl_dtl = $mwk->get_result();

    $sql_sum = "SELECT SUM(qty_aktual) AS total_barang, SUM(harga_aktual) AS total_harga, SUM(amount_aktual) AS total_amount
    FROM pts_detail_aktual
    WHERE idPts = '$id'";
        ...
        <tbody style="border: 0.8px;">
            <tr>
                <td style="height : 450px; vertical-align : top; ">';
                    $sql_dtl = "SELECT model FROM pts_detail_aktual
                    WHERE idPts = '$id' ORDER BY nourut ASC";
                    $mwk = $db1->prepare($sql_dtl);
                    $mwk -> execute();
                    $hsl_dtl = $mwk->get_result();
                    ...
                    $html .='</td>
                <td style="height : 450px; vertical-align : top;"></td>
                <td style="height : 450px; vertical-align : top;">';
                    $sql_dtl = "SELECT ket FROM pts_detail_aktual
                    WHERE idPts = '$id' ORDER BY nourut ASC";
                    $mwk = $db1->prepare($sql_dtl);
                    $mwk -> execute();
                    $hsl_dtl = $mwk->get_result();
                ...
                <td style="height : 450px; vertical-align : top;" align="center">';
                    $sql_dtl = "SELECT qty_aktual FROM pts_detail_aktual
                    WHERE idPts = '$id' ORDER BY nourut ASC";
                    $mwk = $db1->prepare($sql_dtl);
                    $mwk -> execute();
                    $hsl_dtl = $mwk->get_result();
                    while ($sk = $hsl_dtl->fetch_assoc()){
                    $html .= $sk['qty_aktual'] . '<br>';
                    }
                    $html .='</td>
            </tr>
        </tbody>
