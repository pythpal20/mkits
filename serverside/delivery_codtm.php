...
$idpt = $_GET['id'];
if( $idpt == '1'){
    $where = "e.jenis_transaksi = 'KUNJUNGAN' OR e.jenis_transaksi ='TELEPON' ";
} elseif($idpt == '3'){
    $where = "e.jenis_transaksi = 'SHOWROOM '";
} elseif($idpt == '4') {
    $where ="e.jenis_transaksi = 'INTERNAL' ";
} else {
    $where = "e.jenis_transaksi = 'MARKETPLACE' OR e.jenis_transaksi = 'ONLINESTORE'";
}
// } else if($idpt = '2') {
//     $where = "e.jenis_transaksi = 'MARKETPLACE'";
...
$table = <<<EOT
 (
    ...
    WHERE NOT EXISTS (SELECT * FROM customerorder_hdr_delivery d WHERE a.No_Co = d.No_Co) AND ($where) AND YEAR(a.tgl_krm)>'2021' AND a.status = '1' AND a.id_perusahaan ='4'
    GROUP BY b.No_Co
 ) temp
EOT;
